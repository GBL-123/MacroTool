...
- UI is MudBlazor-first: use `Mud*` components (layout, alerts, chips, paper, grid, stack, existing inputs/data grid) themed by `MacroTheme`; keep bespoke markup/CSS only for the brand mark, log terminal, metric hero, and Mud internals that genuinely need tuning. When migrating a region, keep the legacy CSS class on the Mud component so bUnit selectors in `MacroTool.Tests` keep working.
