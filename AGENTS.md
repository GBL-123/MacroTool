...
- UI is MudBlazor-first: use `Mud*` components (layout, alerts, chips, paper, grid, stack, existing inputs/data grid) themed by `MacroTheme`; keep bespoke markup/CSS only for the brand mark, log terminal, metric hero, and Mud internals that genuinely need tuning. `ReconnectModal` is a deliberate exception: it is static SSR fallback UI driven by the framework's `components-reconnect-*` ids/classes and `ReconnectModal.razor.js`, so it must stay hand-written HTML/CSS. When migrating a region, keep the legacy CSS class on the Mud component so bUnit selectors in `MacroTool.Tests` keep working.
