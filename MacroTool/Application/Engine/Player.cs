...
public sealed record PlayerDelegates(
    Func<int, bool, bool, bool> SendKey,
    Func<int, int, bool> SendMouseDown,
    Func<bool> SendMouseUp);

public sealed class Player
{
    private readonly MacroTimeline _timeline;
    private readonly EngineSettings _settings;
    private readonly Func<int, bool, bool, bool> _sendKey;
    private readonly Func<int, int, bool> _sendMouseDown;
    private readonly Func<bool> _sendMouseUp;
    private volatile bool _stop;
    private Thread? _thread;
    private int _round;
    private readonly Stopwatch _clock = new();

    public Player(MacroTimeline timeline, EngineSettings settings)
        : this(timeline, settings, null)
    {
    }

    public Player(MacroTimeline timeline, EngineSettings settings, PlayerDelegates? delegates)
    {
        _timeline = timeline;
        _settings = settings;
        delegates ??= new PlayerDelegates(InputSender.SendKey, InputSender.SendMouseDown, InputSender.SendMouseUp);
        _sendKey = delegates.SendKey;
        _sendMouseDown = delegates.SendMouseDown;
        _sendMouseUp = delegates.SendMouseUp;
    }
    ...
    private void Run()
    {
        ...
        if (keyDown) _sendKey(pendingScan, true, pendingExt);
        if (clickDown) _sendMouseUp();
